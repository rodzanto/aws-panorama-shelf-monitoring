...
function renderItems(items) {
    ...
    for (var i = 0, len = items.length; i < len; i++) {
        if (Counts[i] == 1000) {
            ...
            Purch[i] = Purch[i] + (Counts[i] - items[i].count.N);
            var subtotal = Purch[i] * Price[i];
            if (Purch[i] == (Counts[i] - items[i].count.N)) {
            ...
            if ((Purch[i] - (items[i].count.N - Counts[i])) <= 1) {
                ...
                //Returning items but still one or more in the cart...
                Purch[i] = Purch[i] - (items[i].count.N - Counts[i]);
                var subtotal = Purch[i] * Price[i];
                for (var j = 0; j < Receipt.rows.length; j++) {
                    if (Receipt.rows[j].cells[0].innerHTML == items[i].ProductType.S) {
                        Receipt.rows[j].cells[1].innerHTML = Purch[i];
    ...
    //Recalculate total cost to receipt...
    var lastrow = Receipt.lastElementChild.lastElementChild;
    totalcost = 0;
    for (var j = 0; j < (Receipt.rows.length - 1); j++) {
        if (j >= 1) {
            totalcost = totalcost + parseFloat(Receipt.rows[j].cells[2].innerHTML.substr(1));
        }
    }
